#ifndef OBFUSHEADER_HPP
#define OBFUSHEADER_HPP
...
#if defined(_MSC_VER) and !defined(__clang__) and !defined(__llvm__) // Some clang define _MSC_VER for some reason
#define _MSVC
#elif defined (__GNUC__) or defined(__clang__) or defined(__llvm__)
#define _GNUC
...
// detect arch
#if defined(i386) or defined(__i386__) or defined(__i386) or defined(_M_IX86)
#define x86_32
#elif defined(__x86_64__) or defined(_M_X64) or defined (x86_64)
#define x86_64
...
#if defined(_WIN64) or defined(WIN64) or defined(WIN32) or defined(_WIN32) or defined(__WIN32__) or defined(__NT__)
#define _WINDOWS
#elif  defined(__linux__) or defined(__ANDROID__) // android is linux tbh
#define _LINUX 
...
#elif !defined(OBF_UNSUPPORTED) and !defined(_MSVC) and !defined(_GNUC)
...
#if defined(_WINDOWS) and !KERNEL_MODE
#include <windows.h>
#elif defined(_LINUX) or defined(_ANDROID)
#include <dlfcn.h>
#endif

// Funny. Only makes sense for windows & PE files (Tricks https://github.com/horsicq/Detect-It-Easy)
#if FAKE_SIGNATURES and defined(_WINDOWS) and !KERNEL_MODE
#ifdef _MSC_VER
#pragma section(".arch")
...
#define true_2 ((((_9 + __7() + ((_RND * __2()) * __0()))) / _8) - _1)
#define false_2 ((_3 + __6() + ((_RND * __3()) * _0)) - __9())
#define XOR(x, y) (x + y - (2 * (x & y)))

// Use stored in static memory essential bytes for hardcoded cflow blocks & expressions
#if CFLOW_CONST_DECRYPTION or CFLOW_BRANCHING
...
#define BLOCK_TRUE(block) BLOCK_COND(true_2, block)
#define BLOCK_FALSE(block) BLOCK_COND(false_2, block)
...
// Pointer-based call hiding (Crossplatfworm)
...
#define call_function(ptr, ...) (HIDE_PTR(ptr)(__VA_ARGS__))
#define call_function_default(ptr, ...) (HIDE_PTR(ptr)(__VA_OPT__(,) __VA_ARGS__))

// Symbol-based call hiding (different for Linux & windows)
#if defined(__linux__) or defined(__ANDROID__)
#define CALL_EXPORT(mtd, def, ...) RCAST(def, dlsym(RTLD_DEFAULT, OBF(mtd)))(__VA_ARGS__)
#elif defined(_WINDOWS) and !KERNEL_MODE
...
#if CFLOW_BRANCHING
#define if_2(x) if (true_2) if (int_proxy((long long)(x)) * true_2 and _RND)
#define for_2(x) for (int _i=0; _i<int_proxy(true_2);_i++) for (x)
#define while_2(x) while(int_proxy((long long)(x)) * true_2 and _RND)
#define switch_2(x) switch(int_proxy((long long)(x)) * true_2)
#define return_2 for (int _i=0; _i<RND(1, 100);_i++) return
