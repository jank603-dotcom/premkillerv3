...
namespace ImGui
{
	...
	IMGUI_API void          GlowText( const char* label, ImU32 text_col, ImU32 glow_col, float glow_radius, const ImVec2& pos_text );
	...
	IMGUI_API ImColor       LerpColor( const ImColor& a, const ImColor& b, float t );
	IMGUI_API ImColor       GetHslHashColor( int id );
...
struct ImGuiIO
{
	...
	//   e.g. io.ConfigDebugIsDebuggerPresent = ::IsDebuggerPresent() on Win32, or refer to ImOsIsDebuggerPresent() imgui_test_engine/imgui_te_utilities.cpp for a Unix compatible version).
...
struct ImDrawList
{
	...
	IMGUI_API void  AddRectWH( float x, float y, float w, float h, ImU32 col, float rounding = 0.0f, ImDrawFlags flags = 0, float thickness = 1.0f );
	IMGUI_API void  AddRectFilled( const ImVec2& p_min, const ImVec2& p_max, ImU32 col, float rounding = 0.0f, ImDrawFlags flags = 0 );                     // a: upper-left, b: lower-right (== upper-left + size)
	IMGUI_API void  AddRectFilledWH( float x, float y, float w, float h, ImU32 col, float rounding = 0.0f, ImDrawFlags flags = 0 );
