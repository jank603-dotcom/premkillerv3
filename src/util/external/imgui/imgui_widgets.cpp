...
void ImGui::GlowText( const char* label, ImU32 text_col, ImU32 glow_col, float glow_radius, const ImVec2& pos_text )
{
    ...
    ImVec2 size = ImGui::CalcItemSize( ImVec2( 0, 0 ), label_size.x, label_size.y );
    ImVec2 pos = window->DC.CursorPos + pos_text;

    const ImRect bb( pos, pos + size );
    ImGui::ItemSize( size, style.FramePadding.y );

    ImVec4 glow_color = ImGui::ColorConvertU32ToFloat4( glow_col );

    ImDrawList* draw_list = ImGui::GetWindowDrawList( );
    for ( float radius = glow_radius; radius > 0.0f; radius -= 0.5f )
    {
        float alpha = ( radius / glow_radius ) * glow_color.w;
        ImVec4 current_glow_color = glow_color;
        current_glow_color.w = alpha;
        ImU32 current_glow_col = ImGui::ColorConvertFloat4ToU32( current_glow_color );

        draw_list->AddText( ImVec2( pos.x, pos.y ), current_glow_col, label );
    }

    draw_list->AddText( ImVec2( pos.x, pos.y ), text_col, label );
}
...
#include "include/util/external/imgui/imstb_textedit.h"
